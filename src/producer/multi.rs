...
pub struct MultiProducer<E, C> {
    ...
    /// Shared Producer Context
    /// The Mutex is only taken during Clone and Drop — not on the publish hot path.
    /// Those are lifecycle operations (spawning/destroying a producer), not data-flow operations.
    ///
    /// The hot path (publish → next_sequences → apply_update) never touches shared_producer. It uses:                                                  
    ///     - producer_barrier.compare_exchange() (lock-free CAS) to claim sequences                                                                        
    ///     - producer_barrier.publish() (atomic XOR) to make events visible                                                                                
    ///     - consumer_barrier.get_after() (atomic load) to check consumers  
    ///
    /// So the data plane is fully lock-free. The Mutex is only on the control plane:
    /// creating/destroying producer handles, which happens rarely (startup/shutdown)
    shared_producer: Arc<Mutex<SharedProducerCtx>>, // SAFE cold-mutex here
...
impl<E, C> Drop for MultiProducer<E, C> {
    fn drop(&mut self) {
        let mut shared_ctx = self.shared_producer.lock().unwrap();
        let old_count = shared_ctx.counter.fetch_sub(1, Ordering::AcqRel);
        if old_count == 1 {
    ...
    C: Barrier,
{
    pub(crate) fn new(
        ...
        let shared_producer = Arc::new(Mutex::new(SharedProducerCtx {
            consumer_handles,
