use calvera::{BusySpin, EPolling, Producer, build_uni_producer_unchecked};
...
fn polling(group: &mut BenchmarkGroup<WallTime>, inputs: (i64, u64), param: &str) {
    ...
    let builder = build_uni_producer_unchecked(DATA_STRUCTURE_SIZE, factory, BusySpin);
        ...
        spawn(move || {
            loop {
                match poller.poll() {
                    ...
                    Err(EPolling::NoEvents) => continue,
...
fn processing(group: &mut BenchmarkGroup<WallTime>, inputs: (i64, u64), param: &str) {
    ...
    let mut producer = build_uni_producer_unchecked(DATA_STRUCTURE_SIZE, factory, BusySpin)
